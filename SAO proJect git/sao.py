...
async def info(ctx, user:discord.User):
    try:
        ...
        pdata=[f'{base.hp(user.id)}/{base.hplimit(user.id)}',f'{base.atk(user.id)}',f'{base.accy(user.id)}',f'{base.def_(user.id)}',f'{base.money(user.id)}']
        draw.text((100,250),pdata[0],font=saofont,fill="white")
        draw.text((100,290),pdata[1],font=saofont,fill="white")
        draw.text((100,330),pdata[2],font=saofont,fill="white")
        draw.text((100,370),pdata[3],font=saofont,fill="white")
        draw.text((100,410),pdata[4],font=saofont,fill="white")
        offset = (19,22)
        ...
        if error.__contains__("cannot identify image file") ==True:
            ...
            background.save('get.png')
            await ctx.send(file=discord.File('get.png'))
        elif error.__contains__("User") == True and error.__contains__("not found") == True:
...
async def info_on_error(ctx,error):
    ...
    draw=ImageDraw.Draw(background)
    draw.text((185,75),name,font=chinese,fill="white")
    pdata=[f'{base.hp(ctx.author.id)}/{base.hplimit(ctx.author.id)}',f'{base.atk(ctx.author.id)}',f'{base.accy(ctx.author.id)}',f'{base.def_(ctx.author.id)}',f'{base.money(ctx.author.id)}']
...
@cardinal.command()
async def test2(ctx):
    await ctx.send(base.combo(ctx.author.id))
@cardinal.command()
...
async def bartest(ctx):
    length=round(int(base.hplimit(ctx.author.id))/100)
    wanted=round(int(base.hp(ctx.author.id))/100)
    value=length-wanted
    x="["
    def num(value,x,wanted):
        while wanted > 0:
            x+="⬜"
            wanted-=1
        while value > 0:
        ...
        x+="]"
        return x
    await ctx.send(num(value,x,wanted))
@cardinal.event
...
try:
    ...
    loop.create_task(cardinal.start("圖檔"))
    loop.create_task(crystaln.start("圖檔"))
    loop.run_forever()
